...
# AWS EKS & Managed Infrastructure (Modular Terraform)
...
The infrastructure is partitioned into discrete modules to ensure high availability and clean separation of concerns:

* **VPC Module**: Sets up a multi-AZ network with 3 Public and 3 Private subnets, an Internet Gateway, and a NAT Gateway.
* **Security Module**: Defines a centralized virtual firewall for SSH (Port 22), HTTPS (Port 443), and Application traffic (Ports 80-9000).
* **Compute Module**: Deploys two public EC2 instances for management tools like Jenkins and Ansible.
* **EKS Module**: Provisions the Kubernetes Control Plane and a Managed Node Group using cost-efficient SPOT instances.


---

## 2. File Structure
All commands must be executed from the **Root Directory**. Terraform automatically pulls code from the sub-folders defined in the `module` blocks.
...
└── modules/
    ...
    ├── security/           # Firewall and Security Groups
    ├── compute/            # EC2 Public Instances
